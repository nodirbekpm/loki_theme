<?php
add_action('wp_ajax_filter_by_color', 'handle_filter_by_color');
add_action('wp_ajax_nopriv_filter_by_color', 'handle_filter_by_color');

function handle_filter_by_color() {
    ...
    foreach ($products as $product) {
        ...
        if (!$group || empty($group['color'])) continue;

        $product_colors = is_array($group['color']) ? $group['color'] : [$group['color']];
        $product_color_ids = array_map(function ($term) {
            return is_array($term) ? (int)$term['term_id'] : (int)$term;
        }, $product_colors);

        if (empty($color_ids) || array_intersect($product_color_ids, $color_ids)) {
